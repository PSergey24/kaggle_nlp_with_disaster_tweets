...
class NLP:
    ...
    def main(self):
        ...
        # self.train_lr(train_vectors)
        self.train_rfc(train_vectors)

        sample_submission = self.to_predict(test_vectors, model='rfc')
    ...
    def train_lr(self, train_vectors):
    ...
    def to_predict(self, X_text, model='lr'):
        sample_submission = pd.read_csv("data/sample_submission.csv")

        if model == 'lr':
            sample_submission["target"] = self.model_lr.predict(X_text)
        if model == 'rfc':
